...
<script>
    ...
    const resetTestWorker = async () => {
        if (params.participantId === 'test-worker') {
            ...
            console.log(`Reset user requested but participantId is ${params.participantId}`);
        }
    };
    ...
    // init experiment only if participantId exists
